...
- **Moving Head (DMX fixture) support — v1 color wash (2026-07-16, branch `feat/moving-head-effects`, Design-First Gate explicitly skipped per user "proceed").**
...
### Open decisions
- **Moving Head v1 needs real-xLights validation before merge.** Everything was verified by direct inspection of the generated XML (settings string shape matches the vendor examples exactly), but no automated check can confirm xLights itself renders it correctly — the user should open a generated `.xsq` containing a moving-head group in real xLights and visually confirm the fixture lights up with the expected color per section before this merges to `main`.
- **Moving Head fast-follow candidates** (deliberately out of v1 scope): Pan/Tilt motion via the embedded `Tilt VC: Active=TRUE|Id=...|Type=Ramp|...` value-curve syntax; per-head fan-out (`PanOffset`/`TiltOffset`/`Groupings` > 1) for multi-head formations; short beat-synced shutter-strobe accents on individual heads (observed as ~300ms bursts in both vendor examples, separate from the group-level wash). Each adds real risk (wrong DMX values on real hardware) and should get its own smaller design/test pass rather than being bundled together.
