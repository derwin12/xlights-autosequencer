...
- **Pictures effect support shipped (bug-209).** New `src/generator/image_catalog.py::catalog_images()` recursively scans `<show_dir>/Images` for image/gif files, returning show-relative paths (`Images/name.gif`). New `effect_placer._place_picture_effects()` places them song-scoped (like `_place_video_effect`), cycling each eligible prop (matrix/tree favored, arch/outline/radial excluded per `Pictures`' existing `prop_suitability`) through the whole catalog in 20s segments across the song, seeded by `variation_seed`+prop name so props don't sync to the same image. New `GenerationConfig.picture_effects` flag (default True); new `SequencePlan.picture_effects` field; wired into `plan.py`/`xsq_writer.py`. Deliberately no config-supplied path (avoids repeating bug-192's forgotten-wiring class — directory is derived, not per-song uploaded). Companion advisory feature: `image_catalog.suggest_images_for_words()` fuzzy-matches (stdlib `difflib`) lyric words against catalog filenames, wired into the review analyze flow (`src/review/api/v1/analysis.py`) as a new `image_suggestions` field in the analysis result + persisted session — informational only, doesn't affect generation. 17 new tests (`tests/unit/test_generator/test_picture_effects.py`), 598 total generator+review tests pass, no regressions. **Note:** `builtin_effects.json`'s `Pictures_Filename` storage name (`E_FILEPICKER_Pictures_Filename`) conflicts with what `docs/xlights-effect-params.md` (scraped from real xLights) shows as the current UI control (`E_CUSTOM_Pictures_FilenameBlock`) — kept the existing catalog value for consistency with the rest of the codebase, but **this needs verification against a real xLights-generated sequence** before trusting it in production.
...
### Open decisions
- **Fast-follow explicitly scoped by the user (2026-07-15):** wire `image_suggestions` into generation — when a `_place_picture_effects` placement's time window overlaps a lyric-matched word, prefer that matched image over the random catalog rotation, falling back to random otherwise. Not yet implemented (round 1 was advisory-only by explicit choice).
- **Verify `Pictures_Filename` storage name against real xLights** before relying on generated sequences showing images correctly — see the note under ✅ Done above.
- Whether to add folder-name/theme-mood matching for image selection (explicitly deferred — user chose "flat pool" for round 1).
