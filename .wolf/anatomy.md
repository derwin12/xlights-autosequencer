...
## C:/Users/daryl/AppData/Local/Temp/claude/H--Github-xlight-autosequencer/4c070495-12ed-4b03-b3e3-195d5b91e527/scratchpad/

- `corpus_sweep.py` — Mini-corpus sweep: run the full production crash chain on local songs. (~794 tok)
  - fn `run` L34-73 (~466 tok)
- `crash_proto.py` — Prototype: envelope-area crash detection on an isolated crash stem. (~1130 tok)
  - fn `analyze` L20-108 (~949 tok)
- `crash_proto2.py` — Prototype v2: isolation-weighted crash detection on the crash stem. (~1123 tok)
- `e2e_full.py` — End-to-end: populate StemCache for Dream On, run the real orchestrator, (~931 tok)
- `e2e_stems.py` — End-to-end validation: mp4 -> htdemucs_6s drums -> drumsep cymbals -> stems on disk. (~586 tok)
  - fn `run_model` L17-58 (~450 tok)
- `rescore.py` — Re-score cached sweep cymbals stems with the FINAL detector, printing (~986 tok)

## openspec/changes/crash-stem-impact-score/

- `design.html` — Design: crash-stem impact score (~2884 tok)
- `proposal.md` — Proposal: crash-stem impact score (~919 tok)
- `tasks.md` — Tasks: crash-stem impact score (~439 tok)

## src/

- `cli_old.py` — CLI entry point for xlight-analyze. (~35097 tok)
  - fn `_format_duration` L21-25 (~33 tok)
  - fn `_print_summary_table` L26-44 (~283 tok)
  - fn `_print_breakdown` L45-69 (~374 tok)
  - fn `_rich_error` L70-88 (~188 tok)
  - fn `cli` L89-105 (~224 tok)
  - fn `analyze_cmd` L106-237 (~1425 tok)
  - fn `full_cmd` L238-263 (~219 tok)
  - fn `_run_analysis_from_config` L264-372 (~1396 tok)
  - fn `wizard_cmd` L373-461 (~942 tok)
  - fn `summary_cmd` L462-514 (~619 tok)
  - fn `export_cmd` L515-587 (~728 tok)
  - fn `review_cmd` L588-710 (~1400 tok)
  - fn `scoring_cmd` L711-715 (~32 tok)
  - fn `scoring_list_cmd` L716-735 (~224 tok)
  - fn `scoring_show_cmd` L736-756 (~248 tok)
  - fn `scoring_save_cmd` L757-772 (~166 tok)
  - fn `scoring_defaults_cmd` L773-790 (~244 tok)
  - fn `sweep_cmd` L791-918 (~1427 tok)
  - fn `params_cmd` L919-972 (~607 tok)
  - fn `sweep_suggest_cmd` L973-1016 (~497 tok)
  - fn `sweep_save_cmd` L1017-1045 (~301 tok)
  - fn `stem_inspect_cmd` L1046-1075 (~419 tok)
  - fn `stem_review_cmd` L1076-1114 (~569 tok)
  - fn `sweep_init_cmd` L1115-1201 (~1084 tok)
  - fn `pipeline_cmd` L1202-1252 (~507 tok)
  - fn `export_xlights_cmd` L1253-1325 (~981 tok)
  - fn `sweep_matrix_cmd` L1326-1479 (~1922 tok)
  - fn `sweep_results_cmd` L1480-1576 (~1046 tok)
  - fn `_bar` L1577-1581 (~46 tok)
  - fn `_load_hierarchy_summary` L1582-3020 (~15957 tok)

## src/analyzer/

- `crash_accents.py` — Rare whole-house crash/transient detection (cymbal-stem isolation score). (~2914 tok)
  - fn `detect_crash_accents` L110-231 (~1270 tok)
- `drum_stems.py` — Cymbal separation: chain a drumsep checkpoint on the demucs drums stem. (~2926 tok)
  - fn `default_model_dir` L34-37 (~27 tok)
  - fn `checkpoint_path` L38-41 (~27 tok)
  - fn `ensure_checkpoint` L42-58 (~156 tok)
  - fn `_download_from_gdrive` L59-100 (~525 tok)
  - fn `separate_cymbals` L101-153 (~554 tok)
  - fn `_run_drumsep_inprocess` L154-185 (~343 tok)
  - fn `_load_drumsep_model` L186-202 (~177 tok)
  - fn `_run_drumsep_sidecar` L203-272 (~738 tok)
- `orchestrator.py` — Hierarchy orchestrator: zero-flag pipeline for hierarchical music analysis. (~16332 tok)
  - fn `_md5_file` L34-41 (~57 tok)
  - fn `_output_dir` L42-46 (~43 tok)
  - fn `_hierarchy_json_path` L47-51 (~40 tok)
  - fn `_xtiming_path` L52-56 (~36 tok)
  - fn `_load_cache` L57-97 (~584 tok)
  - fn `_write_cache` L98-124 (~245 tok)
  - fn `_make_stem_algo` L125-133 (~82 tok)
  - fn `_build_algorithm_list` L134-224 (~1018 tok)
  - fn `_get_value_curve` L225-230 (~47 tok)
  - fn `_format_duration` L231-237 (~51 tok)
  - fn `run_orchestrator` L238-808 (~7170 tok)
  - fn `_write_xtiming` L809-843 (~381 tok)
  - fn `_add_mark_layer` L844-867 (~280 tok)
  - fn `_add_section_layer` L868-891 (~238 tok)
  - fn `_section_summary` L892-900 (~83 tok)
  - fn `_collect_onset_times` L901-922 (~237 tok)
  - fn `_nearest_in_sorted` L923-939 (~154 tok)
  - fn `_merge_qm_boundaries` L940-973 (~301 tok)
  - fn `_snap_sections_to_bars` L974-1086 (~1238 tok)
  - fn `_deduplicate_events` L1087-1159 (~692 tok)
  - fn `_filter_events_by_energy` L1160-1217 (~536 tok)
  - fn `_label_energy_tiers` L1218-1259 (~390 tok)
  - fn `_label_beats` L1260-1296 (~462 tok)
  - fn `_derive_eighth_notes` L1297-1331 (~350 tok)
  - fn `_select_beat_with_bpm_check` L1332-1386 (~661 tok)
  - fn `_print_dry_run` L1387-1405 (~258 tok)
- `result.py` — Core data classes for the analysis pipeline. (~10542 tok)
  - class `CriterionResult` L13-51 (~312 tok)
  - class `ScoreBreakdown` L52-93 (~439 tok)
  - class `TimingMark` L94-108 (~106 tok)
  - class `AnalysisAlgorithm` L109-138 (~220 tok)
  - class `TimingTrack` L139-232 (~962 tok)
  - class `LeaderTransition` L233-251 (~148 tok)
  - class `LeaderTrack` L252-279 (~228 tok)
  - class `TightnessWindow` L280-310 (~227 tok)
  - class `TightnessResult` L311-324 (~123 tok)
  - class `SidechainedCurve` L325-354 (~246 tok)
  - class `HandoffEvent` L355-384 (~208 tok)
  - class `InteractionResult` L385-417 (~412 tok)
  - class `ValueCurve` L418-454 (~250 tok)
  - class `ChromaCurve` L455-497 (~356 tok)
  - class `RepetitionGroup` L498-535 (~355 tok)
  - fn `is_hierarchy_schema` L536-550 (~182 tok)
  - class `HierarchyResult` L551-767 (~2930 tok)
  - class `StemSelection` L768-797 (~279 tok)
  - class `ConditionedCurve` L798-832 (~257 tok)
  - class `ValueCurveExport` L833-872 (~321 tok)
  - class `TimingTrackExport` L873-902 (~238 tok)
  - class `ExportManifest` L903-941 (~422 tok)
  - class `AnalysisResult` L942-1014 (~946 tok)

## src/cli/

- `review.py` — Summary, export, review, and chord-stats commands. (~4167 tok)
  - fn `summary_cmd` L29-81 (~619 tok)
  - fn `export_cmd` L82-154 (~728 tok)
  - fn `review_cmd` L155-283 (~1506 tok)
  - fn `chord_stats_cmd` L284-365 (~942 tok)

## src/review/

- `server.py` — Flask review server for xlight-analyze review UI. (~21004 tok)
  - fn `_adapt_hierarchy_for_ui` L26-99 (~674 tok)
  - class `AnalysisJob` L100-137 (~367 tok)
  - fn `_run_analysis` L138-287 (~1852 tok)
  - fn `_progress_generator` L288-324 (~416 tok)
  - fn `_hierarchy_summary_for_server` L325-355 (~387 tok)
  - fn `create_app` L356-1767 (~16561 tok)

## tests/unit/

- `test_crash_accents.py` — Unit tests for src/analyzer/crash_accents.py — cymbal-stem crash detection. (~1429 tok)
  - fn `_full_mix` L19-26 (~99 tok)
  - fn `_cymbal_stem` L27-55 (~298 tok)
  - class `TestDetectCrashAccents` L56-117 (~848 tok)
- `test_drum_stems.py` — Unit tests for src/analyzer/drum_stems.py — cymbal separation wrapper. (~1597 tok)
  - fn `_drums` L15-19 (~53 tok)
  - class `TestSeparateCymbals` L20-105 (~1171 tok)
  - class `TestEnsureCheckpoint` L106-123 (~259 tok)
