...
# ── Riff bursts (2026-07-18) ─────────────────────────────────────────────────
# Short punch on each hierarchy.riff_bursts mark (bass-band onset burst +
# accelerated chord change, see src/analyzer/riff_bursts.py) — a rare
# guitar/bass riff or fill, structurally distinct from the crash punch
# (isolated treble transient), so the pose is deliberately different: a
# lower, wider fan (mid-height tilt, wide PanOffset) instead of the crash's
# near-vertical narrow fan, so the two accent types read differently to an
# audience. Reuses the exact same warmup/existing_placements/vocal-exclusion
# machinery as place_moving_head_crash_accents.
_RIFF_EFFECT_DURATION_MS = 600
_RIFF_VOCAL_EXCLUSION_MS = 500
_RIFF_TILT_DEG = "45.0"
_RIFF_PAN_OFFSET_DEG = "25.0"
_RIFF_LEAD_MS = 300
_RIFF_DIMMER_NOTCH_COUNT = 3


def _build_riff_head_settings(head_count: int, dimmer_curve: str) -> str:
    heads_field = _COMMA_ESCAPE.join(str(i) for i in range(1, head_count + 1))
    return (
        f"Dimmer: {dimmer_curve};"
        f"Wheel: {_COLOR_WHITE};"
        "Shutter: On;"
        f"Pan: 0.0;Tilt: {_RIFF_TILT_DEG};"
        f"PanOffset: {_RIFF_PAN_OFFSET_DEG};TiltOffset: 0.0;"
        "Groupings: 1;Cycles: 1.0;"
        f"Heads: {heads_field}"
    )


def _build_riff_warmup_head_settings(head_count: int) -> str:
    heads_field = _COMMA_ESCAPE.join(str(i) for i in range(1, head_count + 1))
    return (
        f"Pan: 0.0;Tilt: {_RIFF_TILT_DEG};"
        f"PanOffset: {_RIFF_PAN_OFFSET_DEG};TiltOffset: 0.0;"
...
def place_moving_head_riff_bursts(
    layout: Layout,
    hierarchy: HierarchyResult,
    vocal_words: Optional[list[dict]],
    fade_exclusion_start_ms: Optional[int] = None,
    ...
    """Place a short mid-height fan-out punch on the moving-head group at
    each rare riff/fill mark from ``hierarchy.riff_bursts``, preceded by a
    silent warmup when nothing already covers that lead-in window.

    Twin of place_moving_head_crash_accents with a different pose (see the
    constants block above) and a shorter lead-in (riff bursts are a
    detected cluster of onsets, not a single sharp transient, so there's
    less need to be positioned far in advance). Single-song validated so
    far (see src/analyzer/riff_bursts.py docstring) — treat with the same
    caution as a young feature until checked against more songs.
    """
    mh_groups = find_moving_head_groups(layout)
    if not mh_groups or not hierarchy.riff_bursts:
        return {}
    ...
    def _near_vocal(time_ms: int) -> bool:
        return any(
            start - _RIFF_VOCAL_EXCLUSION_MS <= time_ms <= end + _RIFF_VOCAL_EXCLUSION_MS
            for start, end in word_spans
        )
    ...
    for mh_group in mh_groups:
        head_count = len(mh_group.head_names)
        warmup_settings = _build_riff_warmup_head_settings(head_count)
        warmup_params = _build_parameters(
            {i: warmup_settings for i in range(1, head_count + 1)},
            slider_pan=_deg_to_slider(0.0),
            slider_tilt=_deg_to_slider(float(_RIFF_TILT_DEG)),
            slider_pan_offset=_deg_to_slider(float(_RIFF_PAN_OFFSET_DEG)),
        )
        relevant_keys = (mh_group.name, *mh_group.head_names)
        channel_existing = [
            p for key in relevant_keys for p in existing_placements.get(key, [])
        ...
        for mark in hierarchy.riff_bursts:
            ...
            if fade_exclusion_start_ms is not None and mark.time_ms >= fade_exclusion_start_ms:
                continue
            start_ms = max(0, mark.time_ms - _RIFF_LEAD_MS)
            end_ms = min(mark.time_ms + _RIFF_EFFECT_DURATION_MS, hierarchy.duration_ms)
            if end_ms <= start_ms:
                continue
            dimmer_curve = _random_dimmer_curve(mark.time_ms, notch_count=_RIFF_DIMMER_NOTCH_COUNT)
            riff_settings = _build_riff_head_settings(head_count, dimmer_curve)
            params = _build_parameters(
                {i: riff_settings for i in range(1, head_count + 1)},
                slider_pan=_deg_to_slider(0.0),
                slider_tilt=_deg_to_slider(float(_RIFF_TILT_DEG)),
                slider_pan_offset=_deg_to_slider(float(_RIFF_PAN_OFFSET_DEG)),
            )
